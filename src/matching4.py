...
from dataType4 import humanData, itemData
import cv2
import time
				...
				if dist < min_dist:
					if abs(dist - min_dist) < 0.07:
						...
						if calculateDist(feature, h_d.firstFeature) < firstFeatureDist:
							print("first feature",calculateDist(feature, h_d.firstFeature),firstFeatureDist)
							min_dist = dist
							matchCandidate = h_d
					else:
						min_dist = dist
...
def itemMatching(detection, humanDataset, itemDataset):
	...
	for d_n, d_name in zip(detection[0],detection[1]):
		...
		for d_d in itemDataset.values():
			...
			print(d_d.x,dnx,"item distance===: ", np.sqrt((dnx - d_d.x)**2 + (dny - d_d.y)**2), distanceThres+5*(d_d.missing+1))
			if np.sqrt((dnx - d_d.x)**2 + (dny - d_d.y)**2) < distanceThres:# + 5*(d_d.missing+1):
				...
				if d_d.alarm_flag == False and d_d.owner != 0: # no owner object don't update position
					...
					if d_name==d_d.name:
						...
						find_pair = True
						d_d.counter+=1
						print("not updata",d_d.owner,d_d.counter)
						if d_d.counter >10 and d_d.owner==0 and d_d.id not in humanDataset[0].itemList:
							humanDataset[0].itemList.append(d_d.id)
							d_d.update_position(dnx, dny)
		...
		print("item find pair",find_pair,)
		if not find_pair:
	...
	for d_d in itemDataset.values():
		...
		if d_d.updated == False and d_d.missing <= 50:
			...
			if d_d.id==0 and d_d.counter<3:
				d_d.missing==0

			print("d_d onwer",d_d.owner," d_d.missing = ", d_d.id, d_d.name, "count ",d_d.missing)
			# print("\n=\n=\n=")
...
def findClosestHuman(item, humanDataset):
	...
	if closestHuman!=None:
		print("closehuman",closestHuman.id,"posses item id",item.id,item.name)
		...
		#print("human dataset===================================",humanDataset)
	if 0 in humanDataset:
		#print("human dataset111111===================================",humanDataset)
		if item.counter>3 and item.owner==0:
		...
		#print("human dataset item list-----",humanDataset[0].itemList)
		humanDataset[0].itemList.append(0)
		#humanDataset[0].itemList.append(item.id)
