...
def write_results(prediction, confidence, num_classes, nms = True, nms_conf = 0.4):
    ...
    #print("====",output.size(),output.cpu().detach().numpy().tolist())
    write = False
    ...
    for ind in range(batch_size):
        ...
        #print("pred0000",ind,prediction.size(),prediction[ind].cpu().detach().numpy().tolist())
        ...
        seq1=image_pred
        #print("max====",np.array(seq[1].cpu().detach().numpy().tolist()).shape)
        image_pred = torch.cat(seq, 1)
        #print("image2_===",image_pred.size())

        
        #Get rid of the zero entries
        non_zero_ind =  (torch.nonzero(image_pred[:,4]))

        #print("non zero",image_pred[non_zero_ind.squeeze(),:].size())
        #print("==",image_pred[0].size())
        image_pred_ = image_pred[non_zero_ind.squeeze(),:].view(-1,7)
        image_pred_1 = seq1[non_zero_ind.squeeze(),:].view(-1,seq1.size(1))
        ...
        for cls in img_classes:
            ...
            image_pred_class_1=image_pred_1[class_mask_ind].view(-1,seq1.size(1))
            ...
            image_pred_class_1=image_pred_class_1[conf_sort_index]
            ...
            if nms:
                ...
                for i in range(idx):
                    ...
                    image_pred_class_1=image_pred_class_1[non_zero_ind].view(-1,seq1.size(1))

                    image_pred_class=torch.cat((image_pred_class,image_pred_class_1[:,5:seq1.size(1)]),1)
